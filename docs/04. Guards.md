...
Guards (called firewalls in older versions of LmcRbacMvc) are listeners that are registered on a specific event of
...
LmcRbacMvc is flexible enough to allow you to create custom guard. Let's say we want to create a guard that will
...
Now, we just need to add the guard to the `guards` option, so that LmcRbacMvc execute the logic behind this guard. In
