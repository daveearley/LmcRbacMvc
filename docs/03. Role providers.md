...
Roles can come from any sources: in memory, from a file, from a database... However, please note that since LmcRbacMvc
...
Identity providers return the current identity. Most of the time, this means the logged user. LmcRbacMvc comes with a
...
To create a custom role providers, you first need to create a class that implements the `LmcRbacMvc\Role\RoleProviderInterface`
interface.
